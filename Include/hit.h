...
class Hit {
  public:
    ...
    void PrintEnergyTimeStamp(){
      printf("ch: %2d, charge: %u, timestamp: %lu ch, traceLenght: %lu\n", channel, charge, timestamp, traceLenght);
    }
    ...
    void PrintAll(){
      ...
      if( DPPType == DPPType::PHA ) printf("charge: %u, timestamp: %lu, fine_timestamp: %u \n", charge, timestamp, fine_timestamp);
      if( DPPType == DPPType::PSD ) printf("charge: %u, energy_S : %u, timestamp: %lu, fine_timestamp: %u \n", charge, energy_short, timestamp, fine_timestamp);
