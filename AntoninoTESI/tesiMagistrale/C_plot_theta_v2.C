...
//#   update: 27 June 2024, derived from C_plot_theta.C, used to analyze the strange peak in theta
//#           distribution
//###################################################################################################
...
void C_plot_theta_v2(int run)
{
   ...
   char fileInName[50];
   if(run<10){
      sprintf(fileInName, "../Tracks/tracks_run00%i_B.root", run);
   }else if(run <100){
      sprintf(fileInName, "../Tracks/tracks_run0%i_B.root", run);
   }else{
   ...
   histoThetaLastPad->SetLineColor(kGreen);
      ...
      /*if(cl_charge[0] >2000 && cl_charge[1] >2000 && cl_charge[2] >2000 && cl_charge[3] >2000 && cl_charge[4] >2000){
        h_theta_geq2000->Fill(theta_deg);
         ...
         for (int j=0; j<cl_padMult4; j++) {
             if ((a_pads_fired4[j] == 1) || (a_pads_fired4[j] == 2))
                flagLastPad = 1;       
         }
         
         if (flagLastPad != 1)
            histoThetaWithoutLastPad->Fill(theta_deg);
         else
            histoThetaLastPad->Fill(theta_deg);
...
//      }
   ...
   histoTheta->Draw();
   histoThetaWithoutLastPad->Draw("same");
   histoThetaLastPad->Draw("same");
   //h_theta_rms->Draw("same");
   TLegend* l = new TLegend();
   l->SetTextSize(0.035);
   l->AddEntry(histoTheta, "All pads", "l");
   l->AddEntry(histoThetaWithoutLastPad, "Excluding pads 1,2 on row4","l");
   l->AddEntry(histoThetaLastPad, "Only if pads 1,2 on row4 fired","l");
   ...
   TLatex* tex = new TLatex(-50,20, "All rows fired && rms < 2.5");
   tex->Draw("same");
