...
void trackGenerator_row_mult_filter(int run, bool sicFileOpen)
{
...
// Dichiarazione variabili
  ...
  // output file variables
   ...
   Double_t theta012;		// theta row 0,1 & 2 of the track in rad
   Double_t theta012_deg;	// theta row 0,1 & 2 of the track in deg
   
   Double_t theta01;		// theta row 0 & 1 of the track in rad
   Double_t theta01_deg;	// theta row 0 & 1 of the track in deg
   ...
   Double_t chiSquareTheta012;
   Double_t chiSquareTheta01;
...
// other variables
   ...
   double slopeT012, interceptT012, chi2T012;      
   double slopeT01, interceptT01, chi2T01;      
   ...
   UInt_t rowMultiplicity=1; 	// consider tracks with a number of hit row bigger than rowMultiplicity
   ...
   int np012=0; // number of point of the Tgraph
   int np01=0; // number of point of the Tgraph
...
// OPEN output ROOT file //
   char fileOutName[50];
   if(run<10){
         sprintf(fileOutName,"Tracks/tracks_run00%i_rowmult.root",run);
      }else if(run <100){
         sprintf(fileOutName,"Tracks/tracks_run0%i_rowmult.root",run);
      }else{
         sprintf(fileOutName,"Tracks/tracks_run%i_rowmult.root",run);
      } 
   ...
   treeOut->Branch("theta012",&theta012,"theta012/D");
   treeOut->Branch("theta012_deg",&theta012_deg,"theta012_deg/D");
   treeOut->Branch("chiSquareTheta012",&chiSquareTheta012,"chiSquareTheta012/D");   
   treeOut->Branch("slopeT012",&slopeT012,"slopeT012/D");      
   treeOut->Branch("interceptT012",&interceptT024,"interceptT012/D");   


   treeOut->Branch("theta01",&theta01,"theta01/D");
   treeOut->Branch("theta01_deg",&theta01_deg,"theta01_deg/D");
   treeOut->Branch("chiSquareTheta01",&chiSquareTheta01,"chiSquareTheta01/D");   
   treeOut->Branch("slopeT02",&slopeT01,"slopeT01/D");      
   treeOut->Branch("interceptT01",&interceptT024,"interceptT01/D");   
...
// Dichiarazione Histo, Canvas, TGraph and Functions
   ...
   TFitResultPtr fitResultTheta012;  
   TFitResultPtr fitResultTheta01;    
   ...
   TF1 *theta_fit_result012 = new TF1("theta_fit_result 012","[0]+([1]*x)",0.,107.);; // this function is only for drawing the fit result
   TF1 *theta_fit_result01 = new TF1("theta_fit_result 01","[0]+([1]*x)",0.,107.);; // this function is only for drawing the fit result
   ...
   TGraph *grTheta012=new TGraph(0);
   grTheta012->SetMarkerStyle(20);
   grTheta012->SetMarkerSize(1);
   
   TGraph *grTheta01=new TGraph(0);
   grTheta01->SetMarkerStyle(20);
   grTheta01->SetMarkerSize(1);
...
//################### Event loop ######################################
   ...
   //for(int i=0; i<10; i++){
      ...
      if((Timestamp-timeinit)<DeltaT){
         ...
         if(flag[0]+flag[1]>rowMultiplicity){
	   ...
	   //    if(max>100 && (j==0 || j==1 ||j==2)){grTheta012->SetPoint(np012++, zrow[j], cl_x_mm[j]);}
	       if(max>100 && (j==0 || j==1)){grTheta01->SetPoint(np01++, zrow[j], cl_x_mm[j]);}	       
	    ...
	    row[4]->GetYaxis()->SetRangeUser(0,max*2);
          ...
          //  fitResultTheta012=grTheta012->Fit("lin1","SQ");   
            fitResultTheta01=grTheta01->Fit("lin1","SQ");          
   	    if(fitResultTheta01==0){
         ...
         /*      
               interceptT012 = fitResultTheta012->Value(0);
	       slopeT012 = fitResultTheta012->Value(1);
	       theta_fit_result012->SetParameter(0,interceptT012);
	       theta_fit_result012->SetParameter(1,slopeT012);	       
    	       chiSquareTheta012 = fitResultTheta012->Chi2();
    	       //cout<<"intercept "<< interceptT <<"\t slope "<< slopeT << "\t chi2 "<< chiSquareTheta << endl;
	       theta012 = TMath::ATan(slopeT012);
               theta012_deg = theta012*180./TMath::Pi();   */  
               
               
               interceptT01 = fitResultTheta01->Value(0);
	       slopeT01 = fitResultTheta01->Value(1);
	       theta_fit_result01->SetParameter(0,interceptT01);
	       theta_fit_result01->SetParameter(1,slopeT01);	       
    	       chiSquareTheta01 = fitResultTheta01->Chi2();
	       theta01 = TMath::ATan(slopeT01);
               theta01_deg = theta01*180./TMath::Pi();                           
