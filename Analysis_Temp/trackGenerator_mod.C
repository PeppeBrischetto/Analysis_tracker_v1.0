...
//# check that the implicit creation of cluster is correct

//###################################################################################################


void trackGenerator_mod(int run, bool sicFileOpen)
{
...
/*/ OPEN output ROOT file //
   char fileOutName[50];
   if(run<10){
         sprintf(fileOutName,"clusters_tracks_run00%i.root",run);
      }else if(run <100){
         sprintf(fileOutName,"clusters_run0%i.root",run);
      }else{
         sprintf(fileOutName,"clusters_run%i.root",run);
      } 
...
// Dichiarazione Histo, Canvas, TGraph and Functions
   ...
   TCanvas *c1=new TCanvas("c1","c1",1500,50,900,500);
   TCanvas *c2=new TCanvas("c2");
   TCanvas *c3=new TCanvas("c3");
   TCanvas *c4=new TCanvas("c4");
   
   TH2D *bgr= new TH2D("bgr","bgr",60,-0.5,59.5,1000, 0,60000);
   TLegend *leg= new TLegend(0.7,0.65,0.9,0.9);
   leg->AddEntry(row[0], " row 0", "l"); 
   leg->AddEntry(row[1], " row 1", "l");
   leg->AddEntry(row[2], " row 2", "l");
   leg->AddEntry(row[3], " row 3", "l");
   leg->AddEntry(row[4], " row 4", "l");
   TGraph *centroids =new TGraph(5);
   centroids->SetMarkerStyle(22);
   centroids->SetMarkerSize(1);
...
//################### Event loop ######################################
   ...
   //for(int i=0; i<10; i++){
      ...
      if((Timestamp-timeinit)<DeltaT){
	    ...
	    for(int l=0; l<5; l++){
	       centroids->SetPoint(l,cl_x[l], 500);
            }
	 
	    c1->cd();
 	    bgr->Draw();
            row[0]->Draw("hist same");
            row[1]->Draw("hist same");
            row[2]->Draw("hist same");
            row[3]->Draw("hist same");
            row[4]->Draw("hist same");
            centroids->Draw("P");
            leg->Draw();
            c1->Update();
   	    //cout<<"entry "<<i<<endl;
	 ...
	 // Start a new Event and Reser histo and variables
	 timeinit=Timestamp;
	 entryMerged=i;
	 grTheta->Set(0);
	 grPhi->Set(0);
