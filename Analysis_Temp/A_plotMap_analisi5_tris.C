...
void A_plotMap_analisi5_tris(int run)
{
   ...
   int evMult=2;		// row multiplicity event (selct just event with row multiplicity larger than evMult
   ...
   double xcol = 0;
   double zcol = -50;		// starting value for the posizione of the collimator
   const int nh=10;		// number of step the collimator is moved far
   ...
   for (int nu=0;nu<nh;nu++){ 
      sprintf(nomeh,"histo%i",nu);
      TH1F *source = new TH1F(Form("h%d",nu),nomeh,1000,0,100); 
   ...
   for(int i=0;i <entries; i++){
	 ...
	 if(flag[0]+flag[1]+flag[2]+flag[3]+flag[4]>evMult){
	    ...
	    for(int j=0; j<5; j++){
	       ...
	       clusters->SetPoint(j,centroid[j],(j*21.60)+10.80); 	   	       
            }
   ...
   for(int i=0; i<nh; i++){
     cout<<i<<" "<<zcol+i*deltaz<<"  "<<vlist.at(i)->GetMean()<<"\t"<<vlist.at(i)->GetRMS()<<"\t"<<vlist.at(i)->GetRMS()*2.35<<endl;
   }
}
