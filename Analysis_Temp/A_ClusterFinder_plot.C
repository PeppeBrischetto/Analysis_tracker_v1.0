...
void A_ClusterFinder_plot(int run)
{
   ...
   for(int j=0; j< 32; j++){cout<<j<<"\t"<<hit[j]<<endl;}
   ...
   for(int i=0;i <entries; i++){
      ...
      if((Timestamp-timeinit)<DeltaT){
        ...
        //if(Section==4 && Charge>thresh){
           anode->Fill(pad,3,Charge); flag[1]=1;
        ...
        //if(Section==12 && Charge>thresh){
           anode->Fill(pad,1,Charge); flag[3]=1;
        ...
        //if(Section==16 && Charge>thresh){
           anode->Fill(pad,0.1,Charge); flag[4]=1;
        ...
        if(Row==6 && Charge>thresh){
           ...
           for(int g=0;g<62;g++){anodeTime->Fill(g,3.5,Timestamp-timeinit +10);}
        }*/
      }else{
        if(flag[0]>0){
           // decide if it is a single cluster or a multiple one
           for(int j=0; j< 32; j++){
              if(hit[j]!=0 && clusterFlag==0){
                 clusterNumber++;
                 clusterFlag=1;                 
              }
              if(hit[j]==0){
        ...
        cout<<"---------------"<<endl;
        timeinit=Timestamp;
        for(int i=0; i<64; i++){hit[i]=0;}  // zeros the hit array.
        clusterNumber=0;
        clusterFlag=0;
        anode->Reset("ICES");
