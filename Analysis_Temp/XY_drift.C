...
void XY_drift(int run, double z)
{
...
// other variables
   ...
   string filename = "drift_time.txt";  //File to store the values of Z0 and horizontal position resolution
...
// Declaring Histos 
   ...
   TH1F *ply = new TH1F("ply","",6000,0.,10.);  //Histogram with the reconstructed Y coordinate of the colimator
  ...
  TH2D *spot = new TH2D("spot","",20000,-300.0,500.0,600,-50.0,250.0);  //Histogram with the reconstructed beam profile//
   ...
   ply->GetXaxis()->SetTitle("t_{drift} (#mus)");
 ...
 /*  TH2F *bg=new TH2F("bg","",100,0,300,100,-100,400);
  ...
  //printf("Z0: %1.2f,  slope: %1.4f,  intercept: %1.4f \n",z0,slopeT_inv,interceptT_inv);
   ...
   ply->Fill(ycol/50.-0.1,1); //Jitter 100 ns due to electronics
...
/*
   ...
   */
   ...
   cout<<"Drift velocity: "<< 9.9/fitResY->Value(1) <<" cm/us"<<endl;
...
//   file << z0 << " " << resX << " " << fwhmX << " "  << resX2 << " " << fwhmX2 << " "  << resX3 << " " << fwhmX3<< endl;
   ...
   ifstream file(filename.c_str());
   while(!file.eof()) {
   file >> temp1[lineCount] >> temp2[lineCount];
   lineCount++ ;
   }
    
 
   temp1[lineCount-1]=run;
   temp2[lineCount-1]=fitResY->Value(1);

   ofstream file2;  
   file2.open("drift_time2.txt");
    
   for(int lines=0; lines<lineCount; lines++){
   file2 << temp1[lines] << "\t" << temp2[lines] << endl;
   }
    
   file.close();
   file2.close();
   remove("drift_time.txt");
   rename("drift_time2.txt","drift_time.txt");
   }
