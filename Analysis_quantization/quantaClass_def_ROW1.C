...
void quantaClass_def_ROW1(int run){

//################################################################################################################
// Needed variables
   Double_t par[6] = {0.};                                             // parameters-array to store single-fits
   char histoname[100];
   char tit[100];
   Double_t minimum = 0.;
   ...
   TF1 *f1 = new TF1("f1","gaus",104.5,105.7);
   TF1 *f2 = new TF1("f2","gaus",106,108);
   TF1 *f = new TF1("f","gaus(0)+gaus(3)",100,111);
...
// Searching extremes of quantization classes      
   ...
   minimum = f->GetMinimumX(par[1],par[4],1E-8,100000,0);
...
// Quantiazion classes creation - multiplicity correlation
   ...
   TH1D *histo_m[2];                                               // histo-array for the rows
   for(int i=0; i<2; i++){
      ...
      histo_m[i]->SetLineColor(kGreen+1);
      histo_m[i]->SetFillColor(kGreen-5);
      histo_m[i]->SetFillStyle(3001);
      histo_m[i]->SetLineWidth(2);
   }
   
   TH1D *histo_theta[2];                                               // histo-array for the rows
   for(int i=0; i<2; i++){
      ...
      histo_theta[i]->SetLineColor(kGreen+1);
      histo_theta[i]->SetFillColor(kGreen-5);
      histo_theta[i]->SetFillStyle(3001);
      histo_theta[i]->SetLineWidth(2);
   }
   
   TH1D *histo_intercept[2];                                               // histo-array for the rows
   for(int i=0; i<2; i++){
      ...
      histo_intercept[i]->SetLineColor(kGreen+1);
      histo_intercept[i]->SetFillColor(kGreen-5);
   ...
   for(Int_t i=0; i<entries; i++){
      tree->GetEntry(i);
      if(cl_x_mm[1]<minimum /*&& cutG->IsInside(cl_x_mm[0], cl_x_mm[1])*/){
        p1 ++;
        histo_m[0]->Fill(cl_padMult[1]);
        if(cl_padMult[1]==6){
   ...
   cout << "Evts. pop1: " << p1 << "   Evts. pop2: " << p2 << endl;
