//#################################################################################################################
//#   This macro allows to obtain the centre of gravity of each rows from a tracks_ID.root file, thus studying its
//#   behaviour as a function of the thresholds. It refers to a single track event.
//#################################################################################################################
//#   Created December 2025 by A. Pitronaci
//#################################################################################################################
//#   Updated:
//#################################################################################################################

#include <iostream>
#include <vector>
#include <cmath>
#include <TCanvas.h>
#include <TH1F.h>
#include <TRandom.h>
#include <TString.h>
#include <TLegend.h>

#include "../Include/openfiles.h"

using namespace std;
using namespace TMath;

const Int_t NRows = 5;
const Int_t NStrips = 6;
const Int_t NPads = 60;

void cog_v2(int run){

//#################################################################################################################
// Needed variables
   
   char histoname[100];
   char titolox[100];
   Double_t x_i[NRows] = {0.};
   Double_t x[NRows] = {0.};
   Double_t pad[NRows][100] = {0.};
   Double_t charge[NRows][100] = {0.};
   Double_t totalCharge[NRows] = {0.};
   Double_t x_i_provv[NRows] = {0.};
   Double_t x_provv[NRows] = {0.};
   Double_t pad_provv[NRows][100] = {0.};
   Double_t charge_provv[NRows][100] = {0.};
   Double_t totalCharge_provv[NRows] = {0.};
   Double_t threshold[NRows] = {400,400,700,200,200};
   
   TH1D *h_cog[NRows];
   for(int i=0; i<NRows; i++){
      sprintf(histoname,"h_cog %i",i);
      sprintf(titolox,"x_{row:%i}",i);
      h_cog[i]=new TH1D("","",9000,0,300);
      h_cog[i]->GetXaxis()->SetTitle(titolox);
      h_cog[i]->GetXaxis()->SetTitleSize(0.05);
      h_cog[i]->GetXaxis()->SetLabelSize(0.05);
      h_cog[i]->GetXaxis()->SetTitleOffset(0.8);
      //h_cog[i]->GetYaxis()->SetTitle("Counts");
      h_cog[i]->GetYaxis()->SetTitleSize(0.05);
      h_cog[i]->GetYaxis()->SetLabelSize(0.05);
      h_cog[i]->GetYaxis()->SetTitleOffset(1.);
      h_cog[i]->SetNdivisions(7);
      h_cog[i]->SetFillColor(kBlue-5);
      h_cog[i]->SetFillStyle(3001);
   }
   
   TH1D *h_cog_provv[NRows];
   for(int i=0; i<NRows; i++){
      sprintf(histoname,"h_cog_provv %i",i);
      sprintf(titolox,"x_{row:%i}",i);
      h_cog_provv[i]=new TH1D("","",9000,0,300);
      h_cog_provv[i]->GetXaxis()->SetTitle(titolox);
      h_cog_provv[i]->GetXaxis()->SetTitleSize(0.05);
      h_cog_provv[i]->GetXaxis()->SetLabelSize(0.05);
      h_cog_provv[i]->GetXaxis()->SetTitleOffset(0.8);
      //h_cog_provv[i]->GetYaxis()->SetTitle("Counts");
      h_cog_provv[i]->GetYaxis()->SetTitleSize(0.05);
      h_cog_provv[i]->GetYaxis()->SetLabelSize(0.05);
      h_cog_provv[i]->GetYaxis()->SetTitleOffset(1.);
      h_cog_provv[i]->SetNdivisions(7);
      h_cog_provv[i]->SetFillColor(kRed-5);
      h_cog_provv[i]->SetLineColor(kRed);
      h_cog_provv[i]->SetFillStyle(3001);
   }
   
//#################################################################################################################
// Open files
   openTrackFile(run);
   tree->Print();

//#################################################################################################################
// Data loop
   
   for(Int_t i=0; i<entries; i++){
      /*for(Int_t row=0; row<NRows; row++){
         h_cog[row]->Reset();
         totalCharge[row] = 0.;
      }*/
      
      tree->GetEntry(i);
      if(sic_fired==1 && sic_charge>2000){
        for(Int_t row = 0; row < NRows; row++){
           for(Int_t p = 0; p < cl_padMult[row]; p++){
              pad[row][p] = pads_fired[row][p];
              charge[row][p] = pad_charge[row][p];
              totalCharge[row] += charge[row][p];
              x_i[p] = 2.5 + (5 * pad[row][p]);
              x[row] += (x_i[p] * charge[row][p]);
              
              pad_provv[row][p] = pads_fired[row][p];
              charge_provv[row][p] = pad_charge[row][p]-threshold[row];
              totalCharge_provv[row] += charge_provv[row][p];
              x_i_provv[p] = 2.5 + (5 * pad_provv[row][p]);
              x_provv[row] += (x_i_provv[p] * charge_provv[row][p]);
           }
           x[row] = x[row] / (totalCharge[row]);
           x_provv[row] = x_provv[row] / (totalCharge_provv[row]);
           h_cog[row]->Fill(x[row]);
           h_cog_provv[row]->Fill(x_provv[row]);
           totalCharge[row] = 0.;
           totalCharge_provv[row] = 0.;
        }
      }
   }

//#################################################################################################################
// Visualization block
   
   TLegend* l = new TLegend(0.1,0.75,0.45,0.9);
   l->SetTextSize(0.035);
   l->SetLineWidth(0);
   l->AddEntry(h_cog[0], "C.O.G. distrib. + threshold", "f");
   l->AddEntry(h_cog_provv[0], "C.O.G. distrib. - threshold", "f");
   
   TCanvas *c = new TCanvas("c","c",2200,2200);
   c->Divide(3,2);
   for(Int_t row=0; row<NRows; row++){
      c->cd(row+1);
      h_cog[row]->Draw();
      h_cog_provv[row]->Draw("SAME");
      l->Draw("SAME");
   }

}
