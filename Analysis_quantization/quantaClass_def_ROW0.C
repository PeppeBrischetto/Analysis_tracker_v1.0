...
void quantaClass_def_ROW0(int run){
...
// Needed variables
   ...
   TF1 *f1 = new TF1("f1","gaus",62,63.4);
   TF1 *f2 = new TF1("f2","gaus",63.5,66);
   TF1 *f = new TF1("f","gaus(0)+gaus(3)",61,67);
...
// Quantiazion classes creation - multiplicity correlation
   ...
   for(int i=0; i<2; i++){
      ...
      histo_m[i]->SetLineColor(kRed+1);
      histo_m[i]->SetFillColor(kRed-5);
   ...
   for(int i=0; i<2; i++){
      ...
      histo_theta[i]->SetLineColor(kRed+1);
      histo_theta[i]->SetFillColor(kRed-5);
   ...
   for(int i=0; i<2; i++){
      ...
      histo_intercept[i]->SetLineColor(kRed+1);
      histo_intercept[i]->SetFillColor(kRed-5);
