...
// Needed variables
   ...
   Int_t mult[NPads] = {0};                                          // multiplicity-array to cut histograms
   Int_t eTOT[NRows] = {0};
   
   Double_t x_M[NRows] = {0.};
   Double_t x_ccoo[NRows] = {0.};
   ...
   TH1D *histo_xccoo[NRows];                                               // histo-array for the rows
   for(int i=0; i<NRows; i++){
      sprintf(histoname,"histo_xccoo %i",i);
      sprintf(tit,"x_{%d} (mm)",i);
      histo_xccoo[i]=new TH1D("","",9000.,0.,300);
      histo_xccoo[i]->GetXaxis()->SetTitle(tit);
      histo_xccoo[i]->GetXaxis()->SetTitleSize(0.05);
      histo_xccoo[i]->GetXaxis()->SetLabelSize(0.05);
      histo_xccoo[i]->GetXaxis()->SetTitleOffset(0.9);
      histo_xccoo[i]->GetYaxis()->SetTitleSize(0.05);
      histo_xccoo[i]->GetYaxis()->SetLabelSize(0.05);
      histo_xccoo[i]->GetYaxis()->SetTitleOffset(1.);
      //histo_xccoo[i]->SetNdivisions(12);
      histo_xccoo[i]->SetLineColor(kBlue);
      histo_xccoo[i]->SetFillColor(kBlue-1);
      histo_xccoo[i]->SetFillStyle(3001);
      histo_xccoo[i]->SetLineWidth(2);
   }
...
// Graphyical cut definition
   ...
   cutG->SetPoint(0,64.7228,107.77);
   cutG->SetPoint(1,61.3637,104.582);
   cutG->SetPoint(2,61.5236,103.018);
   cutG->SetPoint(3,62.849,102.959);
   cutG->SetPoint(4,65.3397,104.345);
   cutG->SetPoint(5,66.7565,106.443);
   cutG->SetPoint(6,66.4137,107.671);
   cutG->SetPoint(7,64.7228,107.77);
   ...
   for(Int_t i=0; i<entries; i++){
      ...
      if(sic_fired==1 && energySic>2000 /*&& cutG->IsInside(cl_x_mm[0], cl_x_mm[1])*/){
         ...
         for(Int_t pad = 0; pad<cl_padMult[row]; pad++){
            eTOT[row] += mult[pad];
            x_M[row] += cl_x_mm[row]*mult[pad];
         }
         x_ccoo[row] = x_M[row]/eTOT[row];
         histo_xccoo[row]->Fill(x_ccoo[row]);
         }
      }                                                                                 // TCutG-parenthesis
   }
