...
//To run simply open a root session and type: .x C_X_resolution.C(run number,z), where "z" is the distance (in mm) between the plane/line and the tracker //
...
void C_X_resolution(int run, double z)
{
...
// other variables
   ...
   double slopeT123, slopeT04, slopeT024; //Slope of the linear fit Z=f(X) describing the ion track using selective rows
   double interceptT123, interceptT04, interceptT024; //Intercept of the linear fit Z=f(X) describing the ion track (in mm) using selective rows   
...
// Open file
   ...
   tree->SetBranchAddress("slopeT123",&slopeT123);      
   tree->SetBranchAddress("interceptT123",&interceptT123);   

   tree->SetBranchAddress("slopeT04",&slopeT04);      
   tree->SetBranchAddress("interceptT04",&interceptT04);   
  
   tree->SetBranchAddress("slopeT024",&slopeT024);      
   tree->SetBranchAddress("interceptT024",&interceptT024);   
...
// Declaring Histos 
 ...
 /*  TH2F *bg=new TH2F("bg","",100,0,300,100,-100,400);
   ...
   slopeT_inv=(1./slopeT024); //Retrieve the slope of the lines describing ion tracks from the inverse function
   interceptT_inv=-(interceptT024/slopeT024); //Retrieve the intercept of the lines describing ion tracks from the inverse function
