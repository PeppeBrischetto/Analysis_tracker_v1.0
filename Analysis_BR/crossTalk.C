...
// Variables
   ...
   TH2D *scatterDiscr[NRows];
   for(int i=0; i<NRows; i++){
      sprintf(histoname,"scatterDiscr %i",i);
      scatterDiscr[i]=new TH2D("","",50,-2,15,50,-2,15);
      scatterDiscr[i]->GetXaxis()->SetTitle("(x_{3} - f(z_{3})) (mm)");
      scatterDiscr[i]->GetXaxis()->SetTitleSize(0.05);
      scatterDiscr[i]->GetXaxis()->SetLabelSize(0.05);
      scatterDiscr[i]->GetXaxis()->SetTitleOffset(0.8);
      scatterDiscr[i]->GetYaxis()->SetTitle("(x_{4} - f(z_{4})) (mm)");
      scatterDiscr[i]->GetYaxis()->SetTitleSize(0.05);
      scatterDiscr[i]->GetYaxis()->SetLabelSize(0.05);
      scatterDiscr[i]->GetYaxis()->SetTitleOffset(.8);
      scatterDiscr[i]->SetNdivisions(7);
      scatterDiscr[i]->SetFillColor(kBlue-5);
   }
...
// Data loop
   ...
   Double_t x_sc = discr[3]->GetBinContent(i);
   Double_t y_sc = discr[4]->GetBinContent(i);
   scatterDiscr[0]->Fill(x_sc,y_sc);
   ...
   c->cd();
   scatterDiscr[0]->Draw("COLZ4");
   c->SaveAs("Pictures_Analysis/TrackQualityControl/Run52/crossTalk_3_4.png");
