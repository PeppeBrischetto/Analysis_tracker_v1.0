...
int A_eventFinder_plot_tracker_and_sic(int run)
{
   ...
   for(int i=0;i <entriesTracker; i++){
      ...
      if((Timestamp-timeinit)<DeltaT){
        ...
        if(flag[0]+flag[1]+flag[2]+flag[3]+flag[4]>2){
           ...
           for (int j=0;j<entriesSic; j++) {
               treeSic->GetEntry(j);
              
              if ( (TimestampPre-TimestampSic)>timeWindowlow && (TimestampPre-TimestampSic)<timeWindowhigh ) {
        ...
        cout << "\nTotal events detected by the SiC: " << eventSic << endl;
