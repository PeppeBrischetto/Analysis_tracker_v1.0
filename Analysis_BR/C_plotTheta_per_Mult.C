...
void C_plotTheta_per_Mult(int run){
...
// Loop on the tree entries to get theta and filling the TH1D object
   ...
   l = i;
      ...
      for(Int_t j=0; j<60; j++){
         if(cl_padMult[0] == j && cl_padMult[1] == j && cl_padMult[2] == j && cl_padMult[3] == j && cl_padMult[4] == j){
           cout << "theta: " << theta_deg << endl;                             // control line: print vartheta(deg)
